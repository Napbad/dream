...
def test_syntax(dap_main, source_runtime_dir, d_debug = False):
    ...
    for dap_file in dap_files_to_test:
        ...
        elif dap_file.__contains__("type_base_test.dap") or dap_file.__contains__("type_test.dap"):
        ...
        elif dap_file.__contains__("control_test.dap"):
            if not (result.stderr.__contains__("error") or result.stderr.__contains__("warning") or result.stderr.__contains__("Error")):
                print(Fore.RED, "FAIL: with" , result.args, "\nexpect error in stderr" + Style.RESET_ALL)
                print(Fore.YELLOW +"stderr:" + Style.RESET_ALL)
